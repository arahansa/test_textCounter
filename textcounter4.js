...
;(function($) {
  $.textcounter = function(el, options) {
    ...
    base.init = function() {
      ...
      var counterText = base.options.countDown ? base.options.countDownText : base.options.counterText + base.options.max,
      ...
      var tcWrapper = '<' + base.options.countContainerElement + ' class="' + base.options.countContainerClass + '"><span class="' + base.options.textCountClass + '">' + counterNum + '</span>'+counterText+'</' + base.options.countContainerElement + '>';
    ...
    base.setCount = function(count) {
      ...
      if(base.options.hasPrevMarker) {
        var prevElem = base.$el.prev('.' + base.options.countContainerClass);
        if(prevElem.length == 0){
          prevElem = base.$el.prevUntil('.' + base.options.countContainerClass).last().prev();
        }
        $countEl = $(prevElem);
      }else{
  ...
  $.textcounter.defaultOptions = {
    ...
    'max'                       : 20,                      // maximum number of characters/words, -1 for unlimited, 'auto' to use maxlength attribute
